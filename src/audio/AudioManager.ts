...
export class AudioManager {
  ...
  private scheduleDrum(
    ...
    if (endFreq !== startFreq) {
      osc.frequency.exponentialRampToValueAtTime(Math.max(endFreq, 1), time + drum.duration * 0.8)
    }
  ...
  getBeatInfo(): BeatInfo | null {
    if (!this.ctx || !this.bgmPlaying || this.sixteenthDuration <= 0) return null

    const now = this.ctx.currentTime
    // 计算当前步内的进度（0.0 = 刚开始，1.0 = 即将进入下一步）
    const elapsed = now - (this.nextStepTime - this.sixteenthDuration)
    const stepProgress = Math.max(0, Math.min(1, elapsed / this.sixteenthDuration))
