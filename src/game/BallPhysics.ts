...
import { BallState, ColorType, TrackBlock } from '@game/types'
import EventEmitter from '@utils/EventEmitter'

export class BallPhysics extends EventEmitter {
  // 球的状态
  ...
  // 落地块缓存（用于精确销样）
  private lastLandedBlock: TrackBlock | null = null
  ...
  update(deltaTime: number): void {
    ...
    if (this.ball.position.y <= 0.7 && this.ball.isJumping && (currentTime - this.lastLandingTime) > this.minLandingInterval) {
