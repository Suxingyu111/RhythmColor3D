...
export class GameCore extends EventEmitter<GameCoreEvents> {
  ...
  private failOfferTimer: number | null = null;
   ...
   * @param data 落地数据
   */
  private handleBallLanded(data: LandingEvent): void {
    ...
    if (!block) {
      ...
      block = this.trackManager.findNearestBlockAnyLane(
        position.z,
        currentColor,
      );
      if (block) {
  ...
  loadLevel(level: LevelConfig | string): void {
    if (typeof level === "string") {
      ...
      this.currentLevel = this.trackManager.getCurrentLevel() ?? level;
    ...
    const update = () => {
      ...
      if (this.renderer) {
        ...
        if (this.frameCount % 5 === 0) {
          const visibleBlocks = this.trackManager.getVisibleBlocks(
            ballPosition.z,
          );
   ...
   * @param block 方块对象
   */
  private handleBlockHit(block: TrackBlock): void {
    this.blocksHit++;
    this.comboCount++;
   ...
   * @param block 导致失败的方块
   */
  private failGame(_block: TrackBlock | { id: string; color: string }): void {
    ...
    // 延迟 1 秒后判断是否可以复活
    this.failOfferTimer = window.setTimeout(() => {
      this.failOfferTimer = null;
      const ballPos = this.ballPhysics.getPosition();

      if (this.reviveCount < this.maxRevives) {
        // 有复活机会，进入复活选择状态
        this.state = GameState.REVIVING;
        const comboShieldAvailable =
          comboAtDeath >= 10 && !this.comboShieldUsed;
   ...
   */
  reviveGame(method: ReviveMethod): void {
    ...
    this.emit("game-revived", {
      method,
      revivesLeft: this.maxRevives - this.reviveCount,
    });
    console.log(
      `GameCore: Revived via ${method}, ${this.maxRevives - this.reviveCount} revives left`,
    );
  }
  ...
  private clearFailOfferTimer(): void {
    if (this.failOfferTimer !== null) {
      clearTimeout(this.failOfferTimer);
      this.failOfferTimer = null;
    }
  }
