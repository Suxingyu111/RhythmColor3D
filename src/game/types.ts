...
export interface GameResult {
  ...
  completedAt: number;
  reviveCount?: number; // 本局复活次数（失败结算时提供）
}
...
export type ReviveMethod = "ad" | "score" | "shield";
...
export interface SnapToBlockEvent {
  block: TrackBlock;
  position: BallState["position"];
}
...
export interface ReviveOfferEvent {
  revivesLeft: number;
...
export interface GameRevivedEvent {
  method: ReviveMethod;
  revivesLeft: number;
}

export type BallPhysicsEvents = {
  landed: [LandingEvent];
  "color-changed": [ColorType];
  "snapped-to-block": [SnapToBlockEvent];
  "boost-activated": [BoostEvent];
  "boost-deactivated": [];
};

export type GameCoreEvents = {
  "level-loaded": [LevelConfig | string];
  "game-started": [];
  "game-update": [GameUpdateEvent];
  "game-finished": [GameResult];
  "game-failed": [GameResult];
  "game-revive-offer": [ReviveOfferEvent];
  "game-revived": [GameRevivedEvent];
  "block-hit": [BlockHitEvent];
  "boost-activated": [BoostEvent];
  "boost-deactivated": [];
  "ball-color-changed": [ColorType];
  "lane-changed": [{ lane: number }];
  "game-paused": [];
  "game-resumed": [];
  error: [Error];
};
