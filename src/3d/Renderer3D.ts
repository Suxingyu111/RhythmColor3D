...
export class Renderer3D {
  ...
  private initLandingParticlePool(): void {
    ...
    this.landingParticleGeo.setAttribute('position', new THREE.BufferAttribute(positions, 3))
    ...
    this.landingParticles = new THREE.Points(this.landingParticleGeo, this.landingParticleMat)
  ...
  private initBoostParticlePool(): void {
    ...
    this.boostParticleGeo.setAttribute('position', new THREE.BufferAttribute(positions, 3))
    this.boostParticleGeo.setAttribute('size', new THREE.BufferAttribute(sizes, 1))
    ...
    this.boostParticles = new THREE.Points(this.boostParticleGeo, this.boostParticleMat)
  ...
  startBoostParticles(): void {
    if (!this.boostParticles || !this.boostParticleMat || !this.boostParticleGeo || !this.boostParticleVelocities) return
    ...
    // 同步粒子颜色为球体当前颜色
    if (this.ballMesh) {
      const ballColor = (this.ballMesh.material as THREE.MeshStandardMaterial).color.getHex()
      this.boostParticleMat.color.setHex(ballColor)

      // 关键：每次激活时重置所有粒子位置到球周围，防止二次激活时粒子不可见
      const posAttr = this.boostParticleGeo.getAttribute('position') as THREE.BufferAttribute
  ...
  private updateBoostParticles(): void {
    if (!this.boostActive || !this.boostParticleGeo || !this.boostParticleVelocities || !this.ballMesh) return
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (relZ < -40 || relZ > 40 || Math.abs(relX) > 35 || relY < -3 || relY > 28) {
        // 重生到球周围随机位置（偏前方）
  ...
  private initTrail(): void {
    ...
    this.trailGeometry.setAttribute('aSize', new THREE.BufferAttribute(sizes, 1))
    this.trailGeometry.setAttribute('aOpacity', new THREE.BufferAttribute(opacities, 1))
  ...
  private updateLandingParticles(): void {
    if (!this.landingAnimating || !this.landingParticleGeo || !this.landingParticleMat || !this.landingVelocities) return

    const elapsed = Date.now() - this.landingStartTime
    const t = Math.min(elapsed / this.PARTICLE_DURATION, 1.0)
    const posArray = (this.landingParticleGeo.getAttribute('position') as THREE.BufferAttribute).array as Float32Array
    ...
    (this.landingParticleGeo.getAttribute('position') as THREE.BufferAttribute).needsUpdate = true
  ...
  playBallBounceEffect(): void {
    ...
    const animateCushion = () => {
      ...
      if (t < 0.3) {
        ...
        // 弹性回弹
        const bt = (t - 0.3) / 0.7
        this.ballBounceOffsetY = cushionDepth * (1 - bt) * Math.cos(bt * Math.PI * 0.5)
      }
      if (t < 1.0) {
        requestAnimationFrame(animateCushion)
      } else {
  ...
  playRippleEffect(blockId: string): void {
    ...
    const blockColor = (mesh.material as THREE.MeshStandardMaterial).color.getHex()
    ...
    // 收集所有需要清理的对象
    const disposables: { obj: THREE.Object3D; geo?: THREE.BufferGeometry; mat?: THREE.Material }[] = []
    ...
    for (let i = 0; i < sparkCount; i++) {
      ...
      if (i < 24) {
        ...
        if (edge === 0) { ex = -halfW + ratio * baseW; ez = -halfD }
        else if (edge === 1) { ex = halfW; ez = -halfD + ratio * baseD }
        else if (edge === 2) { ex = halfW - ratio * baseW; ez = halfD }
        else { ex = -halfW; ez = halfD - ratio * baseD }
        ...
        // 速度：纯水平向外扩散，不向上
        const nx = edge === 1 ? 1 : edge === 3 ? -1 : (Math.random() - 0.5) * 0.5
        const nz = edge === 0 ? -1 : edge === 2 ? 1 : (Math.random() - 0.5) * 0.5
    ...
    const animate = () => {
      ...
      if (origMat.emissiveIntensity > origEmissive) {
        ...
        origMat.emissiveIntensity = origEmissive + (3.0 - origEmissive) * (1.0 - flashT * flashT)
      }
  ...
  playPadBounceEffect(blockId: string): void {
    ...
    const animate = () => {
      ...
      if (t < 0.3) {
        ...
        const elastic = 1 - Math.pow(1 - bounceT, 3) * Math.cos(bounceT * Math.PI * 2)
  ...
  playLandingParticlesAtBall(): void {
    if (!this.ballMesh || !this.landingParticleGeo || !this.landingParticleMat || !this.landingVelocities || !this.landingParticles) return
  ...
  playBallExplodeEffect(): void {
    ...
    const animateRedFlash = () => {
      const t = Math.min((Date.now() - flashStart) / flashDuration, 1.0)
      this.ambientLight.color.lerpColors(new THREE.Color(0xff2222), savedAmbientColor, t)
      this.ambientLight.intensity = 1.5 + (savedAmbientIntensity - 1.5) * t
      if (this.bloomPass) {
        this.bloomPass.strength = 1.5 + (savedBloomStrength - 1.5) * t
      }
      if (t < 1.0) requestAnimationFrame(animateRedFlash)
    }
    requestAnimationFrame(animateRedFlash)
  }
   ...
   * 获取或创建缓存 Material
   */
  private getCachedMaterial(hexColor: number, isColorChanger: boolean): THREE.MeshStandardMaterial {
  ...
  createTrackBlock(block: TrackBlock): THREE.Mesh {
    ...
    // 分裂方块（正方形）顶面贴白色箭头
    if (block.isSplit && this.arrowTexture) {
      const arrowGeo = new THREE.PlaneGeometry(0.5, 0.6)
  ...
  updateBallState(ballState: BallState, beatInfo?: BeatInfo | null): void {
    ...
    this.visualBallX += (this.targetBallX - this.visualBallX) * Math.min(this.LANE_LERP_SPEED * dt, 1)

    this.ballMesh.position.x = this.visualBallX
    this.ballMesh.position.y = ballState.position.y + ballState.position.z * this.trackSlope + this.ballBounceOffsetY
    this.ballMesh.position.z = ballState.position.z

    this.ballMesh.rotation.set(ballState.rotation.x, ballState.rotation.y, ballState.rotation.z)
    ...
    if (beatInfo) {
      ...
      if (this.ballPointLight) {
        if (beatInfo.isStrongBeat && beatInfo.stepProgress < 0.1) {
          ...
          this.ballPointLight.intensity += (0.8 - this.ballPointLight.intensity) * 0.1
        }
      }
   ...
   */
  updateCameraPosition(ballPosition: { x: number; y: number; z: number }, beatInfo?: BeatInfo | null): void {
    ...
    const smoothing = 0.1
    this.camera.position.x += (targetCameraX - this.camera.position.x) * smoothing
    this.camera.position.y += (targetCameraY - this.camera.position.y) * smoothing
    this.camera.position.z += (targetCameraZ - this.camera.position.z) * smoothing
    ...
    if (this.cameraShakeDecay > 0) {
      ...
      this.cameraShakeDecay = Math.max(0, this.cameraShakeDecay - this.currentDt / 0.3)
    }
